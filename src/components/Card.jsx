...
function CardOuvrier() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/users')
      ...
      .then(data => {
        ...
        const filtered = data.filter(user => user.role === 'service_provider');
        setWorkers(filtered);
      })
  ...
  return (
    <div className="d-flex flex-wrap gap-4">
      {workers.map(worker => (
        <Card key={worker._id} style={{ width: '22rem' }} className="mb-4 shadow-sm">
          <Card.Body>
          ...
          <Card.Img
            variant="top"
            src={'./assets/images/img.png'} // par défaut ou selon logique image backend
            style={{ height: '180px', objectFit: 'contain', padding: '1rem' }}
          />
          <ListGroup className="list-group-flush">
            <ListGroup.Item>
              <FontAwesomeIcon icon={faTools} className="me-2 text-primary" />
              <strong>Métier:</strong> {worker.servicesCategory || 'Non spécifié'}
            </ListGroup.Item>
            <ListGroup.Item>
            ...
            <ListGroup.Item>
              <div className="d-flex align-items-center">
                ...
                <div>
                  {renderStars(worker.avis || 4)} {/* note fictive si non disponible */}
                  <span className="ms-2 small text-muted">({(worker.avis || 4).toFixed(1)})</span>
                </div>
              </div>
            </ListGroup.Item>
          </ListGroup>
